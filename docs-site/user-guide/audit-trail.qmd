...
Every write to a regulated table — samples, custody, results, signatures —
lands in an append-only audit trail. Nothing the application does escapes it,
because the trail is written by database triggers, not by application code that
