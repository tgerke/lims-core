...
![A sample's tests and results: versioned entries, four-eyes verification, and a QC verdict per result.](../images/04-sample-detail.png){.screenshot fig-alt="lims-core sample detail showing several ordered tests, each with versioned results, an entered-then-verified status, and an in-spec or out-of-spec QC column"}
...
[electronic signature](signatures.qmd). Where results are produced in batches on
an instrument run, the [analytical testing](analytical-testing.qmd) page covers
