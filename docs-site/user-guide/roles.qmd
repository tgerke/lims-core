...
[disposal of a specimen](biobank-operations.qmd) is a supervisor-only authority,
separate from ordinary sample handling. The lab-wide
[reagent inventory](inventory.qmd) is authorized on an `inventory.manage`
authority held in any study, because reagents are not study-scoped. And the
[QC review board](quality-control.qmd) is deliberately read-only — it introduces
