...
![The biobank actions on a sample record: store, aliquot, and derive.](../images/04b-sample-biobank.png){.screenshot fig-alt="lims-core sample detail for a whole-blood specimen showing volume remaining and the aliquot and derive panels"}
...
disposal — appends to the [chain of custody](storage-and-custody.qmd) and the
[audit trail](audit-trail.qmd). None of them edits an existing row; the record
only ever grows.
:::
