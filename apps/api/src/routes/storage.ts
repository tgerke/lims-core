import { moveSample, storeSample, withActor } from "@lims-core/core";
import { samples } from "@lims-core/db";
import { moveRequestSchema, storeRequestSchema } from "@lims-core/schemas";
...
export const storageRoutes: FastifyPluginAsync = async (app) => {
  ...
  // Interactive freezer-map placement (ADR-0015): place or relocate a sample to a
  // specific cell, recording storage_remove at the old cell on a move.
  app.post("/samples/:sampleId/move", async (request, reply) => {
    const { sampleId } = request.params as { sampleId: string };
    const user = request.user;
    if (!user) return reply.code(401).send({ error: "authentication required" });
